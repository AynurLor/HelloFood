    ...
    private val callback = OnMapReadyCallback { googleMap ->
                    ...
                    if (dataSnapshot.childrenCount.toString() == "7") {
                        ...
                        database.child(Firebase.auth.currentUser!!.uid).get().addOnSuccessListener {
                            if (it.exists()) {
                                ...
                                googleMap.addMarker(MarkerOptions().position(currentAddress).title(getString(R.string.title_order_address)))
        ...
        btn_setAddress.setOnClickListener {
            ...
            database.child(Firebase.auth.currentUser!!.uid).updateChildren(user).addOnSuccessListener {
                Toast.makeText(context, R.string.toast_text_location, Toast.LENGTH_SHORT).show()
            }.addOnFailureListener {
                Toast.makeText(context, R.string.toast_text_location_failed, Toast.LENGTH_SHORT)
                    .show()
            }
        }
        ...
        googleMap.setOnMapClickListener {
            ...
            val markerOptions = MarkerOptions().position(it).title(getString(R.string.toast_text_order_address))
            googleMap.clear()
