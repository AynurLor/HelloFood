...
class ProfileFragment : Fragment() {
    ...
    private fun deleteUser() {
        ...
        alertDialog.setMessage(R.string.text_message_you_sure)
        alertDialog.setPositiveButton(R.string.text_ok, DialogInterface.OnClickListener { dialogInterface, i ->
            ...
            user.delete().addOnCompleteListener { task ->
                if (task.isSuccessful) {
                    ...
                    Toast.makeText(context, R.string.toast_text_account_deleted, Toast.LENGTH_SHORT).show()
                    auth.signOut()
                    startActivity(Intent(context, MainActivity::class.java))
                    activity?.finish()
                } else {
                    Toast.makeText(context, R.string.toast_text_something_wrong, Toast.LENGTH_SHORT).show()
                }
            }
        })
    ...
    private fun updateData() {
        ...
        database.child(Firebase.auth.currentUser!!.uid).updateChildren(user).addOnSuccessListener {
            Toast.makeText(context, R.string.toast_text_profile_updated, Toast.LENGTH_SHORT).show()
        }.addOnFailureListener {
            Toast.makeText(context, R.string.toast_text_profile_updated_failed, Toast.LENGTH_SHORT)
                .show()
        }
    }
