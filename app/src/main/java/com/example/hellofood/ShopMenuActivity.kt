            ...
            override fun onDataChange(dataSnapshot: DataSnapshot) {
                if (dataSnapshot.exists()) {
                    if (dataSnapshot.childrenCount.toString() != "7") {
                        ...
                        alertDialog.setMessage(R.string.text_message_select_address)
        ...
        if (shopId == cartId) {
            if (!quantity.equals("0")) {
                ...
                database.child("carts").child(Firebase.auth.currentUser!!.uid)
                    ...
                    .setValue(cart)
                    .addOnSuccessListener {
                        Toast.makeText(this, R.string.toast_text_added_cart, Toast.LENGTH_LONG).show()
                    }
                    .addOnFailureListener {
                        Toast.makeText(baseContext, R.string.toast_text_failed, Toast.LENGTH_LONG)
                            .show()
                    }
            } else {
                Toast.makeText(this, R.string.toast_text_select_quantity, Toast.LENGTH_SHORT).show()
            }
        } else {
            val alertDialog = AlertDialog.Builder(this)
            alertDialog.setTitle(R.string.notify_ups_title)
            alertDialog.setMessage(R.string.toast_text_clear_cart)
            alertDialog.setPositiveButton(R.string.text_ok, { dialogInterface: DialogInterface, i: Int -> })
            alertDialog.show()
        }
